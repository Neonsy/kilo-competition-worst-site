type HostilityPhase = 1 | 2 | 3;

export const MAXIMUM_HOSTILITY = {
  visual: {
    resonanceIntensity: 0.97,
    resonanceNoiseSeverity: 0.88,
    resonancePulseIntensity: 0.97,
    resonanceFragmentDensity: 'dense' as const,
    resonanceCoverage: 'full' as const,
  },
  shell: {
    profile: 'heavy' as const,
    intensity: 0.95,
    breakMs: 340,
    healMs: 620,
    maxTargets: 5,
    ambientRangeMs: [1200, 2200] as [number, number],
    aftershockDelayMs: 220,
  },
  shellAmbientCycle: {
    intervalRangeMs: [680, 1320] as [number, number],
    breakChance: 0.76,
    maxTargets: 3,
    aftershockChance: 0.32,
  },
  heartbeat: {
    intervalMs: 1300,
    spikeIntervalMs: 3600,
    baseStrength: 0.62,
    spikeStrength: 0.9,
  },
  entryGate: {
    introTrackUrl: '/audio/intro/intro.mp3',
    fadeOutLeadMs: 2000,
    progressHoldAt100Ms: 600,
    fallbackDurationMs: 12000,
    introVolume: 0.36,
    chaosProfile: 'aggressive-readable' as const,
    requireExplicitEntry: true,
    entryLabel: 'Click to Enter Museum',
    showOncePerTab: false,
    sessionKey: 'mobd:gate-v6-seen-v6',
    minDurationMs: 8000,
    maxDurationMs: 15000,
    phaseTickRangeMs: [320, 860] as [number, number],
    falseCompleteChance: 0.34,
    regressionChance: 0.28,
    stallChance: 0.4,
    releaseGlitchMs: 620,
  },
  overlay: {
    intensity: 'high' as const,
    occluderBaseChance: 0.34,
    occluderWindowMs: [900, 1700] as [number, number],
    badges: 12,
    ghosts: 14,
    rifts: 6,
    fractureNoticeIntervalMs: 1100,
    gifMadness: {
      assets: [
        { src: '/media/hostility/gifs/1.gif', weight: 4 },
        { src: '/media/hostility/gifs/2.gif', weight: 3 },
        { src: '/media/hostility/gifs/3.gif', weight: 2 },
        { src: '/media/hostility/gifs/4.gif', weight: 1 },
        { src: '/media/hostility/gifs/5.gif', weight: 1 },
      ] as const,
      maxActiveDesktop: 4,
      maxActiveMobile: 3,
      spawnIntervalRangeMs: [520, 1080] as [number, number],
      burstChance: 0.58,
      burstExtraCountRange: [1, 2] as [number, number],
      ttlRangeMs: [2200, 5000] as [number, number],
      opacityRange: [0.24, 0.58] as [number, number],
      scaleRange: [0.72, 1.18] as [number, number],
      sizeRangePx: [96, 232] as [number, number],
      rotationRangeDeg: [-18, 18] as [number, number],
      edgePaddingPx: 16,
      targetSelectors: [
        '[data-trap-zone]',
        'button',
        'a',
        'input',
        'select',
        'textarea',
        '.res-shell',
        '.mobile-hostile-panel',
      ] as const,
    },
  },
  primitives: {
    cursorTrap: 0.44,
    cursorHotspotOffset: 8,
    focusSabotage: 0.34,
    clipboardTrap: 0.28,
    dragFriction: 1.6,
    dragSnapBack: 0.22,
    chromeNoise: 0.9,
    chromeFakeControl: 0.38,
    cursorMisclassification: 0.7,
    cursorDesync: 0.42,
    cursorGhostCount: 3,
    loadingLoop: 0.42,
    loadingFalseComplete: 0.38,
    loadingRegression: 0.31,
    loadingStall: 0.36,
    loadingStageDurationMs: [430, 760] as [number, number],
    cursorPresentation: {
      nativeCursorPolicy: 'mostly-visible-brief-hide' as const,
      hideBurstChanceOnDesync: 0.16,
      hideBurstWindowMs: [140, 280] as [number, number],
      hideBurstCooldownMs: 1100,
      trailLifetimeMs: 3000,
      trailSpawnIntervalMs: 70,
      maxTrailNodesDesktop: 42,
      maxTrailNodesMobile: 24,
      trailBlurPxRange: [6, 14] as [number, number],
      trailOpacityRange: [0.08, 0.42] as [number, number],
      trailPointerStyle: 'real-pointer' as const,
      trailScaleRange: [0.9, 1.2] as [number, number],
      trailRotationJitterDeg: [-6, 6] as [number, number],
      liveDecoyEnabled: true,
      liveDecoyCountDesktop: 1,
      liveDecoyCountMobile: 0,
      liveDecoyOffsetPxRange: [10, 24] as [number, number],
      criticalControlRemapChance: 0.6,
      criticalControlRemapIntervalMs: [480, 980] as [number, number],
      desyncClickOffsetChance: 0.34,
      desyncClickOffsetPxRange: [4, 12] as [number, number],
    },
  },
  tour: {
    beforeValidateChance: 0.56,
    afterValidateChance: 0.52,
    beforeTransitionChance: 0.54,
    idleChance: 0.46,
    regressionChance: 0.24,
    lockoutRangeMs: [2200, 4200] as [number, number],
    randomValidationAdditive: 0.1,
    skillChaos: {
      enabled: true,
      comboWindowMs: 9000,
      comboDecayMs: 6500,
      maxCombo: 6,
      contractRewardTokenCap: 2,
      contractSuspicionPenalty: 3,
      contractScoreReward: 15,
      catastrophicTokenDowngradeEffect: 'instability' as const,
      autoTokenAtPityAttempt: 4,
    },
  },
} as const;

export function fixedPhaseRecord<T>(value: T): Record<HostilityPhase, T> {
  return { 1: value, 2: value, 3: value };
}
